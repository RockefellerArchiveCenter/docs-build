...
def call_command(command):
    try:
        ...
        print(
            "Error calling `{command}`: {output}".format(
...
class Site:
    def __init__(self, site_config):
        ...
        self.repositories_dir = os.path.join(
            config['site_root'], config['repositories'])
    ...
    def update_data_file(self, data_file):
        ...
        yaml_config['github_repo'] = 'https://github.com/RockefellerArchiveCenter/{0}'.format(
            self.current_repo)
        with open(data_file, 'w') as f:
            yaml.safe_dump(yaml_config, f, default_flow_style=False)

    def get_updated_date(self):
        out = subprocess.Popen(["git --git-dir={0}/.git show --format=%ci".format(
            self.current_repo_dir)], stdout=subprocess.PIPE, shell=True)
